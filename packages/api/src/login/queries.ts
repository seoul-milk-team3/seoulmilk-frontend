...
import { postLogin, postBranchLogin } from ".";
import { axiosInstance } from "../instance";
import { REFRESH_TOKEN_KEY, USER_ID_KEY } from "@seoulmilk/utils";
import { PostLoginRequestBody, PostBranchLoginRequestBody } from "./types";
 ...
 * 대리점 로그인 Mutation 훅
 */
export const useBranchLoginMutation = (setError: any) => {
  const navigate = useNavigate();

  return useMutation({
    mutationFn: (data: PostBranchLoginRequestBody) => postBranchLogin(data),
    ...
    onError: (error: AxiosError) => {
      ...
      if (status === HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR) {
        setError("businessId", { message: "입력하신 사번을 찾을 수 없어요!" });
        setError("password", { message: "비밀번호가 올바르지 않아요." });
        return;
      }
    },
